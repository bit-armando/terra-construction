...
type FormData = Omit<TeamMember, "id"> & { id?: string };

const emptyForm: FormData = { name: "", role: "", photo: "", phone: "", zone: "" };
...
export default function AdminEquipoPage() {
  ...
  function openEdit(item: TeamMember) { setForm({ ...item }); setError(""); setDialogOpen(true); }
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/team/${deleteId}`, { method: "DELETE", credentials: "same-origin" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-2xl font-bold text-foreground">Equipo</h1>
          <p className="text-muted-foreground">Asesores que aparecen en la sección &quot;Nosotros&quot;.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">No hay asesores registrados.</div>
      ) : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-serif">{form.id ? "Editar asesor" : "Nuevo asesor"}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.role} onChange={(e) => upd("role", e.target.value)} placeholder="Asesor de ventas" />
            </div>
            <div className="space-y-2">
              <Label>Teléfono (WhatsApp)</Label>
              <Input value={form.phone} onChange={(e) => upd("phone", e.target.value)} placeholder="5214421234567" />
            </div>
            <div className="space-y-2">
              <Label>Zona</Label>
              <Input value={form.zone || ""} onChange={(e) => upd("zone", e.target.value)} placeholder="Querétaro Norte" />
            </div>
